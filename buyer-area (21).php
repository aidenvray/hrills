...
<td style="vertical-align: top;width: 14.2%;">
    '.(($ddp_basics !== null && $ddp_basics !== '') 
        ? '<b>' . number_format((int)$ddp_basics, 0, '', ' ') . ' $</b>' 
        : '—').'
</td>    
    ...
    <div class="rqq-status-block">      
        ...
        <div class="rqq-actions-row">
            ...
            <span data-project_id="'.$row->ID.'" class="view-quotes">
                ...
                <span class="vq-text">Offers</span>
            </span>
...
if (!empty($bids)) {
    ...
    foreach ($bids as $bid) {
        ...
        /* ===== DELIVERY ===== */
        echo '<td>
            <div style="font-size:14px;"><b>'.$bid->days_done.' days</b></div>
            </td>';
        ...
        if (!empty($attachments)) {
            ...
            if ($mime === 'application/pdf') {
                echo '<td>
                    <div><a href="'.$url.'" target="_blank" class="rqq-pdf-clean">📄 View PDF</a></div>
                 </td>';
            } else {
                echo '<td>
                    <div style="font-size:13px;color:#999;">No file</div>
                 </td>';
            }
...
/* ===== ACTIONS ===== */
        echo '<td>
            <div class="rqq-actions-pro">
                ...
                <span class="open-message-modal rqq-pro-chat"
                    data-rfq="'.$pid.'" 
                    data-supplier="'.$bid_user.'">
                    Chat
                </span>
...
jQuery(document).ready(function($){
    ...
    $(document).on('click', '.view-quotes', function(e){
        ...
        var $desc = $quotes.next('.rqq-quote-desc');

        // убрать +badge → показать общее число
        var totalCount = $row.data('bid-count');
        var $quotesCell = $row.find('td').eq(3);
        var $badge = $quotesCell.find('.quote-badge');

        if ($badge.length) {
    $quotesCell.html(
        '<div style="display:flex; gap:5px;">' +
            '<span class="quote-count" data-count="'+totalCount+'">'+totalCount+'</span>' +
        '</div>'
    );
}
